...
<head>
    <?php
         ...
         require 'manage_statistic/menu/time_format.php';
            ...
            $date = date("Y");
    $sql_search_month="SELECT datatime,sum(money),sum(meter_united) FROM meter_management INNER JOIN house ON meter_management.house_id=house.house_id  WHERE datatime like '$date%' GROUP BY MONTH(datatime),YEAR(datatime) ORDER BY datatime ";
...
<script type="text/javascript">
    $(function () {
        $('#hc_container').highcharts({
            ...
            title: {
                text: 'กราฟแสดงประวัติใช้น้ำ ประจำพ.ศ.<?php echo $date;?>'
            },
