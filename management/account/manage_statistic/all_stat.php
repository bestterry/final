...
$sql_search_month="SELECT datatime,sum(money),sum(meter_united) FROM meter_management   WHERE (datatime between '$year_before-$month_before-01' and '$year_after-$month_after-31')  GROUP BY MONTH(datatime),YEAR(datatime) ORDER BY datatime ";
$objq_search=mysqli_query($conn,$sql_search_month);

$sql_search_sum="SELECT sum(meter_united),sum(money) FROM meter_management   WHERE (datatime between '$year_before-$month_before-01' and '$year_after-$month_after-31')";
